...
/* var BNKNO=$(html).find("select[name='BKDT_BNKNO']").val()
var CUSID=$(html).find("select[name='BKDT_CUSID']").val()
...
/* UI_getdata($("#PrcsID").val(),BNKNO,CUSID+'|'+STDATE+'|'+ENDATE,"","","LSW_SGETBNTBALANCEGRID") */
...
function LoadBSAStatus(html){
	...
	$(html).find(".CallbackStatusHolder").html($(op).find("RESULT").text()+'<span style="font-weight:700">'+$(op).find("FLG").text()+'</span>');
