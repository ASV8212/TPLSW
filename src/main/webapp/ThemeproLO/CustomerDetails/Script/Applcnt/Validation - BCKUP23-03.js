...
//var Resi=$($('.BankDetail1').find(event.target).closest('.DYNROW')[0]).find("input[name='CEMI_ECUAMNO']:checked").val();
	...
	//	if($($('.BankDetail1').find(event.target).closest('.DYNROW')[0]).find("input[name='CEMI_ECUAMNO']:checked") == true){
		...
		$("#CEMI_EUAMNO"+row).next().find(".MndtryAstr").html("*");
		$("#CEMI_EUAMNO"+row).attr("disabled",false);
	...
	if ($("#CEMI_UDYAMVERI"+row).val()=='Verified')
	{
	$("#CEMI_ECUAMNO"+row).attr("disabled",true)
	}
	else
	{
	$("#CEMI_ECUAMNO"+row).attr("disabled",false)
	}
...
{
		if ($("#CDOG_UDYAMVERI").val()=='Verified')
		{
		$("#CDOG_CUAMNO").attr("disabled",true)
		}
		else
		{
		$("#CDOG_CUAMNO").attr("disabled",false)
		}
		...
		{
		$("#CDOG_UAMNO").val('');
		$("#CDOG_UAMNO").removeClass("CEMIMndtry");
		$("#CDOG_UAMNO").next().find(".MndtryAstr").html("*");
		$("#CDOG_UAMNO").attr("disabled",true);
		$(".veri").hide();
		}
		else
		{
		$("#CDOG_UAMNO").addClass("CEMIMndtry");
		$("#CDOG_UAMNO").next().find(".MndtryAstr").html("*");
		$("#CDOG_UAMNO").attr("disabled",false);
		$(".veri").show();
		}
}
