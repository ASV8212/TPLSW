...
function viewfile() {
	...
	for(i=0; i<$("input:checked" ).length;i++)
	{
	var IOP='http://'+window.location.hostname+LoadFrmXML("RS006");
	var ChkSelect = $("#tabledata").find($('input:checkbox[name=select_al]:checked'))[i];
...
function viewfldtls() {
	...
	var redirect= IOP+"ThemeproLO/Common/Pages/Attachment/DocumentDetails.jsp?processid="+processid+ "&url=" + url + "&versnno=" + versnno+ "&flsize=" +flsize;
...
var domain2= window.location.hostname+LoadFrmXML("RS006");
