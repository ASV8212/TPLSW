...
**/
		...
		if(PrcsID!="" && formid!=""){
			$(".Lkuphide").hide();
		$("#PropPrcsID").val(PrcsID);
		  $("#FormId").val(formid);
	var Username = GetCurrentUser();//'david'//
	var AttchDetls = UI_getdata(formid,PrcsID,Username,"","","Sam_sViewAttch");	
	if($(AttchDetls).find("Id").text()=="null"){$("#CusIdPropNo").val('');}
	else{	$("#CusIdPropNo").val($(AttchDetls).find("Id").text());}

	
	$("#BrchID").val($(AttchDetls).find("BrId").text());

	$("#WrkflwName").val($(AttchDetls).find("Wfid").text());
	AttachDMSFetch(PrcsID,formid);
		}
		else
			{
			$(".Lkuphide").show();
