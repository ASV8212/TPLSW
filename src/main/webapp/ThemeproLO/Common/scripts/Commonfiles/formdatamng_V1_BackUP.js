...
function CheckValidPanKyc(Id,Verify,FstNam,LstNam,CusType,CUSID,Addr1,Addr2,INCM1,INCM2,PROFILE,EMPADDRI,EMPADDRII)
{
	if($("#"+Verify).val()!="Verified")
	{
	  IdVal=$("#"+Id).val()
...
function DupPanval(Prcsid,PAN,Constitution,CUSID)
{
	...
	if(PANVAL != "")
	{
	  if($("#"+Constitution).val() != "Partnership Firm")
	   {
		var xml=UI_getdata(PrcsId,PANVAL,cusidVAL,"","","LSW_SCHKDUPDATA");
...
if($($(html).find("." + FieldsClassName)[i]).is("select")==true || $($(html).find("." + FieldsClassName)[i]).is("input")==true){
...
if(FileAccept == 'NO')
{
	alert("Image format Photo only  Upload and file size should not less than 5kb")
	return
}
...
function KYCDELETE(Verify,FldId,Type,Attachment,CUSID)
{	
...
//Place At Last
function PosdxChk(CUSID,Verify,Type,ExLon)
{	
