...
function GSTAnalysisFn(Gst,CusName,FrmDate,ToDate)
{   
      var OP="";
	  
	  if($("#"+Gst).val()=="" || $("#"+CusName).val()=="" || $("#"+FrmDate).val()==""  || $("#"+ToDate).val()=="" )
	  {
      ...
      var Param1="";
    	   ...
    	   $.ajax({
               ...
               complete: function OnSuccess_submit(xml1) {
				...
				var OPxml=UI_getdata($("#PrcsID").val(),OP,"","EMAIL","","LSW_SSENDGSTANALYSISLNK");
...
function CheckKYCDetl(Event,Id,Verify,Type,HTML)
{
   ...
   if(Type=="IFSC")
	 {
	   var AcctNo=$(HTML).find("input[name="+Fld2+"]").val()	
	   if(AcctNo=="")
	   {
	...
	if(Fld5=="MULTIEMP") 
	   {
		$(HTML).find("[data-Validatearia="+Fld1+"]").text('Verified');
 	    $(HTML).find("[data-Validatearia="+Fld1+"]").removeClass("btn-yelInplain");	
 		$(HTML).find("[data-Validatearia="+Fld1+"]").removeClass("btn-RedInplain");
 		$(HTML).find("[data-Validatearia="+Fld1+"]").addClass("btn-GrnInplain"); 
		$(HTML).find("input[name="+Fld1+"]").val('Verified')
...
$.ajax(settings).done(function (response) {
	$(HTML).find("."+Fld2).show();
	$(HTML).find(".UAMNupload1").hide();
	$(HTML).find(".UdyamView1").show();
	$(HTML).find("input[name="+Fld2+"]").val(response);
		 ...
		 if (code.test(code_chk) == false) {
			...
			$("[data-Validatearia="+Verify+"]").text('Upload Failed');
             ...
             return false;
        	    ...
        	    var str=obj["result"][0].details.name.value.replace("&", "AND");
					str=str.replace(/["']/g, "");
        	    var Data=str.replace(" ","|");
       ...
       //  var code = /([C c])/;
         ...
         if (code.test(code_chk) == false) {
	        ...
	        var xml=UI_getdata($("#PrcsID").val(),str+'|'+FatherName+'|'+Address+'|'+DOB,"PAN Upload",CusID,"","LSW_TINSKARZADETL");
			  ...
			  else if(Type=="AADHAAR XML VERIFY")
			  {
			     $(".AVER").show();
			...
			var xml=UI_getdata($("#PrcsID").val(),str+'|'+FatherName+'|'+Address+'|'+DOB,"Aadhar XML Upload",CusID,"","LSW_TINSKARZADETL"); 
			  }
			  else if(Fld5=="PASSPORT")
			  {
	    	    ...
	    	    if(FileType!=true)
				 ...
				 $("#"+Fld6).val(obj["result"][0].details.fileNum.value);
				 if($("#"+Fld6).val()!="")
				 {
					$("#"+Fld6).next().addClass('active'); 
				 }
				 ...
				 FatherName=obj["result"][0].details.father.value
			var xml=UI_getdata($("#PrcsID").val(),str+'|'+FatherName+'|'+Address+'|'+DOB,"Passport Upload",CusID,"","LSW_TINSKARZADETL");
			  }
			  else if(Fld5=="VoterID")
			  {
	    	    ...
	    	    if(FileType!=true)
				 ...
				 var str=obj.result[0].details.name.value.replace("&", "AND").replace("-","");
					str=str.replace(/["']/g, "");
				...
				var xml=UI_getdata($("#PrcsID").val(),str+'|'+FatherName+'|'+Address+'|'+DOB,"Voter ID Upload",CusID,"","LSW_TINSKARZADETL");
			 ...
			 else if(Fld5=="Aadhar")
			  {
	    	    ...
	    	    if(FileType!=true)
				 ...
				 // var DOB=obj["result"][0].details.dob.value
				  ...
				  $(".AVER").show();
				  var xml=UI_getdata($("#PrcsID").val(),CusName+'|'+FatherName+'|'+Address+'|'+DOB,"Aadhar Upload",CusID,"","LSW_TINSKARZADETL");
			  }
			  else if(Fld5=="DL")
			  {
				var FileType=obj.result[0].type;
...
function AutomPaym(LonID,Amt,GSTAmt,Page)
{
	...
	$.ajax({
        url: "/TPLSW/Atom",
        type: 'POST',
        data: {Param1:LoanID,Param2:"Service|"+Page,SPNAME:"LSW_SATOMAPICREATE",PRCSID:$("#PrcsID").val(),Prvnt:$(window.parent.parent.document).find("#Prvnt").val()},
        async:false,
        //dataType: 'json',
       //contentType:'application/json',
        success: function(stm){        
...
else
{
    ...
    else if(Type=="status")
     {
			...
			var ver=UI_getdata("",$("#PrcsID").val(),$("#"+Cusid).val(),Category,"OTP Verification","LSW_SSMSEMAILVERIFY"); 		  
		...
		if($("#"+VerifyId+Row).val()!="")
		  {
		    ...
		    $(HTML).find("."+Attachfield).text('Verify');
			$(HTML).find("."+Attachfield).addClass("btn-yelInplain");
		    $(HTML).find("."+Attachfield).removeClass("btn-GrnInplain");
		    $(HTML).find("."+Attachfield).removeClass("btn-RedInplain");
		  }
		}
		else
		{
...
function UniqueNoPOPUP(Evnt){
	...
	else
	{
	  ...
	  if($("#DMY5").val().split("|")[2] != "SendToCredit" && $("#DMY5").val().split("|")[2] != "PreLoginSB" && $("#DMY5").val().split("|")[2] != "PreLogin"  )
	{
             ...
             // $(html).find("#"+name).attr("id",name + j);
			  ...
			  if($(html).find('.MULTILOADMULTI').length > 0)
			  {
				  ...
				  for(var multiload = 0;multiload<MULTILOADMULTIlen;multiload++)
				  {
					  ...
					  multiloadhtml = LoadMultiData_Supporting (multiloadTblName,multiloadPrcsVal,multiloadUniqNo,multiloaddv,multiloadFieldClass,multiloadSP)
...
function LoadMultiData_Supporting (TblName,PrcsVal,UniqNo,DV,FieldClass,SP)
{
	//$("#"+PRCSID).val(PrcsVal)
...
$(html).find("#" + name).val(CURCommaSep(parseFloat(val.replace(/,/g,"")).toFixed(0)));
          ...
          if (y == true) {
             ...
             // $(html).find("#"+name).attr("id",name + multiloadj);
              ...
              //$("."+DV).append(html);
              finalhtml=finalhtml+$(html)[0].innerHTML
