...
/*
function CheckPayMode(html,id)
{	
var PayMode=$("#"+id). val();
	
          if(PayMode=="RTGS"||PayMode=="NEFT"||PayMode=="Cheque")
           {
           $(html).find(".RTGS").show();
...
function GentrateDisbMemo()
{
	...
	var flname = IOP+LoadFrmXML("RT087")+"&__format=pdf&@PARAM1="+PrcsId+"&@PARAM2="+Scheme+"&@PARAM3="+Tranche+"&__filename=DM_"+$("#DMY7").val().split("|")[7]+".pdf";
...
function CHECKPAYMENTAMT(Mode,AMT)
{
	...
	if(PaymentAmt<200000 &Mode1=='RTGS')
		...
		$("#"+Mode).val('');
		//$("#"+Mode).empty();
        //$("#"+Mode).append('<option value="" selected>--Select--</option><option value="RTGS">RTGS</option><option value="Cheque">Cheque</option><option value="NEFT">NEFT</option><option value="DD">DD</option>') 
...
function CalcMnthFrRepy(){
	...
	if($("#DMY7").val().split('|')[8] == "HE02" && $("#DMY7").val().split('|')[0] == "GECL RST")
	{
		IntrYr = 24
	}
...
{   
	...
	if($("#"+Mode).val()=="Cheque" || $("#"+Mode).val()=="DD")
	{
		 $(HTML).find(".OTHACC").removeClass('DBFDMndtry')
		 $(HTML).find(".CUSACC").removeClass('DBFDMndtry')
		 $(HTML).find(".CUSACC ").next().find(".MndtryAstr").html("");
