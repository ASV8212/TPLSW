...
function ChechUpldURLMain(HTML,Cnt)
{
	var URLVal=$(HTML).find("input[name='MNGQ_ATTCHURL']").val()
  	
	if(URLVal=="")
	{
	  $(HTML).find(".MNGQ_ATTCHURL").hide()
	  $(HTML).find(".rounded").hide()
	}
	else
	{
	  $(HTML).find(".MNGQ_ATTCHURL").show()
	  var FileName=URLVal.split("\\")[1]
	  $(HTML).find("span[name='MNGQ_URLFILENAME']").text(FileName)
	  $(HTML).find(".rounded").show()
	}
}


function ChechUpldURL(HTML,Cnt)
{
	var URLVal=$(HTML).find("input[name='MQMR_ATTACHURL']").val()
  	
	if(URLVal=="")
	{
	  $(HTML).find(".MQMR_ATTACHURL").hide()
	  $(HTML).find(".rounded").hide()
	}
	else
	{
	  $(HTML).find(".MQMR_ATTACHURL").show()
	  var FileName=URLVal.split("\\")[1]
	  $(HTML).find("span[name='MQMR_URLFILENAME']").text(FileName)
	  $(HTML).find(".rounded").show()
	}
}
...
function QryTO()
{
	...
	if(Person=="To Same Person")
		{
		 $(".QryTo").hide();
		}
	else
		{
		var Data=UI_getdata($("#PrcsID").val(),$("#MNGQ_FRMUSRID").val(),"","","","LSW_SGETQUERYTO")
		$("#MNGQ_TOUSRIDREOP").empty();
	    $("#MNGQ_TOUSRIDREOP").append($(Data).find("RESULT").html());
	    $('#MNGQ_TOUSRIDREOP').material_select();
...
function DocFldUpldHndlrResul(id,docu,KYCName,UploadView)
{
