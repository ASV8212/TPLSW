...
function GetSalesDrpDwnVal()
{	 
	 var URLNAVCHK = UI_getdata($('#LogUsr').val(),$($('.DashHdrDataVal').find('.active')[0]).text().trim(),"","","","LSW_SGETSALESDRPDWNVAL")	 
	 var DrpVal=''	 
	 $("#CATEGORY").html("")
	 for ( i=0;i<$(URLNAVCHK).find('a').length;i++)
	 {
  DrpVal= '<option value="'+$($(URLNAVCHK).find('a')[i]).find('Category').text() +'">'+ $($(URLNAVCHK).find('a')[i]).find('Category').text()+'</option>'
	 ...
	 var a=$($(URLNAVCHK).find('a')[2]).find('Type').text()
