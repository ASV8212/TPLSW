...
{	 
	 var URLNAVCHK = UI_getdata($('#LogUsr').val(),$($('.DashHdrDataVal').find('.active')[0]).text().trim(),"","","","LSW_SBRDRPDOWNDATA")
...
function SalesDashGrid()
{
	...
	$("#BTNAcctRecv").click();
	var a=$($('.DashHdrDataVal').find('.active')[0]).text().trim()
    Colname.textContent = a
...
//GridComp
 ...
 function ChkPrimLoanID()
 {
	  ...
	  var LBSI_PRIMLONID=$("#LBSI_PRIMLONID").val()
		var xml=UI_getdata(LBSI_PRIMLONID,$("#LBSI_PRODUCT").val(),"","","","LSW_SCHKPRIMARYLOANID")
...
function DshBrdData2(id)
{
	...
	if($($('.DashHdrDataVal').find('.active')[0]).text().trim()=="Region" || $($('.DashHdrDataVal').find('.active')[0]).text().trim()=="Zone")
	{


	$("#CATGRY").val($($(id).closest('tr').find('td')[11]).text());
	$("#CATUSRID").val($($(id).closest('tr').find('td')[1]).text());
	}
	else
	{
...
function BSMRPTCHNG(VAL)
{
	...
	else if(VAL=='Reportee')
	{
		...
		$('#Area').closest('li').removeClass('active');
	    $('#Region').closest('li').removeClass('active');
	    $('#Zone').closest('li').removeClass('active');					
	...
	$('#BTNAcctRecv').click();
	$('.branch').show();
	$('#Area').closest('li').addClass('active');
	$('#Reportee').closest('li').removeClass('active');
	...
	$('#BTNAcctRecv').click();
	$('.branch').show();
	$('#Region').closest('li').addClass('active');
	$('#Area').closest('li').removeClass('active');
	$('#Reportee').closest('li').removeClass('active');
	$('#Branches').closest('li').removeClass('active');
	...
	$('.branch').show();
	$('#Region').closest('li').removeClass('active');	
	$('#Zone').closest('li').addClass('active');
	$('#Area').closest('li').removeClass('active');
	$('#Reportee').closest('li').removeClass('active');
	$('#Branches').closest('li').removeClass('active');
...
function GridControlDSHBRDWTHHREFLNG (popTableModPageGrid1,TableID,dtData,dtcolumn,hideClm)
{
	 popTableModPageGrid1 = $('#'+TableID).DataTable({ 
...
function DashbrdOnChng()
{
if($('.FILETAT').hasClass('Active')==true)
{
	$('.FILETAT').hide();
	$('.FILETAT').removeClass('Active');
	$('.WTHPNDYSTRDAY').show()	
	$('.WTHPNDYSTRDAY').addClass('Active');	
}	
else if($('.WTHPNDYSTRDAY').hasClass('Active')==true)
{
	$('.WTHPNDYSTRDAY').hide();
	$('.WTHPNDYSTRDAY').removeClass('Active');
	$('.FILETAT').show()	
	$('.FILETAT').addClass('Active');	
}	
}


function GridControlDSHBRDAPPLNO (popTableModPageGrid1,TableID,dtData,dtcolumn,hideClm)
{
	 popTableModPageGrid1 = $('#'+TableID).DataTable({ 
...
"aoColumnDefs": [ 
	   ...
	   { targets: 0, "render": function ( data, type, row, meta ) {                            
			    ...
			    var rowno = meta.row;	
				var HTML =	'<span class="InitWF1"><input type="text" style="display:none"  value="'+data+'" id="APPLID|'+rowno+'"  name="APPLID" maxlength="10" class="form-control ">';			
				HTML = HTML + '<div class="HyperControls">'
				HTML = HTML + '<a type="button" class="Btxt4" name="'+data+'" data-toggle="modal" onclick="DSHBRDAPPLNODATA(this);" data-target="#ADDKYCDocModal" href="#">'+data+'</a>'
				HTML = HTML + ' </div></span>'				
				HTML = HTML + '</span>'; 
...
function DSHBRDAPPLNODATA(id)
{
	
var ActIdINDX=$(id).closest('table').find('td:eq('+$($('th:contains("ActivityID")')[0]).index()+')').index()
var PRCSIdINDX=$(id).closest('table').find('td:eq('+$($('th:contains("ProcessID")')[0]).index()+')').index()

	//$(location).attr('href',window.location.origin + "/TPLSW/Applcnt+"?PrcsID="
	
	$(location).attr('href',window.location.origin + "/TPLSW/Applcnt"+"?PrcsID="
+$($(id).closest('tr').find('td')[PRCSIdINDX]).text()
+"&ActvID="+$($(id).closest('tr').find('td')[ActIdINDX]).text()
+"&PrMs9=&PrMs10=FormMainTab1&PrMs1=View");         

//http://10.9.44.33:7073/TPLSW/Applcnt?PrcsID=89726_HomeEqutity_process&ActvID=172449_89726_HomeEqutity_process_BranchOPS&PrMs9=FormPageTab1&PrMs10=FormMainTab1

	
}

						
function GetSalesDrpDwnVal(BrName)
{	 
	 var URLNAVCHK = UI_getdata($('#LogUsr').val(),$($('.DashHdrDataVal').find('.active')[0]).text().trim(),BrName,"","","LSW_SGETSALESDRPDWNVAL")	 
	 var DrpVal=''	 
	 $("#CATEGORY").html("")
	 for ( i=0;i<$(URLNAVCHK).find('a').length;i++)
	 {
  DrpVal= '<option value="'+$($(URLNAVCHK).find('a')[i]).find('Category').text() +'">'+ $($(URLNAVCHK).find('a')[i]).find('Category').text()+'</option>'

		 $("#CATEGORY").append(DrpVal)
		 var a=$($(URLNAVCHK).find('a')[i]).find('Type').text()
       		 
	 }
	  $('#CATEGORY').material_select();
	 
    Gridcol.textContent = a
}

function DrpDwnHdShw()
{
		
	if($($('.DashHdrDataVal').find('.active')[0]).text().trim()!='')
	{
    $('.DRPDWN').show()
...
function BHMENUCHNG(val)
{
		
var MENUVAL=$(val).text();
	
	$(location).attr('href',window.location.origin + "/TPLSW/Home"+"?PrMs5="+MENUVAL);         
...
function BHMENUCHK()
{
	var len=$(".BHMENU").find('li').length	
var par= getUrlParam("PrMs5");

if(par=="")
{
$($(".BHMENU").find('li')[0]).addClass('active')
...
if(len>0)
{
$(".BHMENU").find('li').removeClass('active')
	
for(i=0;i<=len;i++)
{	
if($($(".BHMENU").find('li')[i]).text().trim()==par)
{
$($(".BHMENU").find('li')[i]).addClass('active')
}
}
...
function GridControlACTNBLEDSHBRDAPPLNO (popTableModPageGrid1,TableID,dtData,dtcolumn,hideClm)
{
	 popTableModPageGrid1 = $('#'+TableID).DataTable({ 
...
"aoColumnDefs": [ 
	   ...
	   { targets: 0, "render": function ( data, type, row, meta ) {                            
			    ...
			    var rowno = meta.row;	
				var HTML =	'<span class="InitWF1"><input type="text" style="display:none"  value="'+data.split('|')[1]+'" id="APPLID|'+rowno+'"  name="APPLID" maxlength="10" class="form-control ">';			
				HTML = HTML + '<div class="HyperControls">'
				HTML = HTML + '<a type="button" class="Btxt4" name="'+data.split('|')[0]+'" data-toggle="modal" onclick="ACTNBLEDSHBRDAPPLNODATA(this);" data-target="#ADDKYCDocModal" href="#">'+data.split('|')[0]+'</a>'
				HTML = HTML + ' </div></span>'				
				HTML = HTML + '</span>'; 
...
function ACTNBLEDSHBRDAPPLNODATA(id)
{
	var MENUVAL=$(id).closest('span').find('input[type=text]').val().replace(/andamp;/g,'&');

	var index=$(id).closest('td').index()
	var fieldname=$($(id).closest('table').find('thead th')[index]).text();
	var Header=$("#ACTNNAME").val().split(":")[0];
	if((Header=="Disbursal Numbers " && fieldname=="NAME")||(Header=="Login Numbers " && fieldname=="NAME"))
	{
	SubActionableBtn(id);	
	}
	else{
	$(location).attr('href',window.location.origin + MENUVAL);         
	}
}


function FileTAT(Event)
{
	
	$("#DataPerfRptIPopup").click();
	FncallMyApplDshBrd(this,'Table8',{spname:'LSW_SFILETAT',DBSrc:'currentProfile',TableHeader:'card-headerGridAsh',Mode:'',Param:$('#LogUsr').val(),brid:Event,MnuId:''},{0:$('#LOCC_BrID'),1:$('#LOCC_BrName')},'||','DSHBRDWTHHREF');

}


	function SubActionableBtn(Id) {
		var BrName=$(Id).attr('name')
	    	$("#BrName").val(BrName)
			$("#ACTNNAMEI").val($("#ACTNNAME").val())
		    $("#ACTNBRSubPopup").click();

         FncallMyApplDshBrd(this,'Table12',{spname:"LSW_SACTNABLEDASHGRIDPOUP",DBSrc:"currentProfile",TableHeader:"card-headerGridAsh",Mode:"",Param:$("#LogUsr").val(),brid:$("#ACTNNAMEI").val().split(":")[1],MnuId:$("#ACTNNAMEI").val().split(":")[0]+'|'+$("#BrName").val()},{0:"",1:""},"||","SUBACTNBLEDSHBRDAPPLNO");

	    }
		
		
function GridControlSUBACTNBLEDSHBRDAPPLNO (popTableModPageGrid1,TableID,dtData,dtcolumn,hideClm)
{
	 popTableModPageGrid1 = $('#'+TableID).DataTable({ 
