	...
	$(location).attr('href',encodeURI(window.location.origin + "/TPLSW/"
		...
		/*$('#LBSI_LONTYPE').on('change', function() {
           ...
           $(document).on("click","#Search",function() {
			   ...
			   {
		          FncallMyAppl(this,'Table3',{spname:'LSW_SMYAPPLICATIONDTLSQ',DBSrc:'currentProfile',TableHeader:'card-headerGridAsh',Mode:'',Param:$('#LogUsr').val(),brid:$('#DMY4').val().split("|")[0]+'|'+$('#FromDate').val()+'|'+$('#ToDate').val()+'|'+$('#ApplNo').val()+'|'+$('#Name').val()+'|'+$('#AggrNo').val(),MnuId:$("#VERTICAL").val()},{0:$('#LOCC_BrID'),1:$('#LOCC_BrName')},'||3','GENERALQUEUE');
			   }
			   else if($(".FormMainTab1 li.active").text()=="Disbursed")
			   {
				   FncallMyAppl(this,'Table3',{spname:'LSW_SMYAPPLICATIONDTLSOPS',DBSrc:'currentProfile',TableHeader:'card-headerGridAsh',Mode:'',Param:$('#LogUsr').val(),brid:'Loan'+'|'+$('#FromDate').val()+'|'+$('#ToDate').val()+'|'+$('#ApplNo').val(),MnuId:$("#VERTICAL").val()},{0:$('#LOCC_BrID'),1:$('#LOCC_BrName')},'||3,8,9','OPS');
			   }
	        });
...
$(document).on("click",".MigWF1",function() {
//$(".MigWF1").click(function () {
	var OldProcessID = $($(this).closest("tr").find("td")[6]).text();
	var Vertical=$("#VERTICAL").val();
	...
	$.ajax({
	            ...
	            url: "/TPLSW/WFINIT",
	            //dataType: "json",
	            data: {XML : "<OldProcessID>"+OldProcessID+"</OldProcessID>",Prvnt : $("#Prvnt").val()},
	            async: true,	      
	            type: "POST",
	            success: function(res) {
	            ...
	            if	(res.split("~")[0] == "Workflow Initiated")
	            	{
						...
						var OverAllStatus="";
						var WFACTVINIT = "";
						var ACTVTY=["PDD","OTC","Sanction","BranchOPS"]
						for(var j=0;j<ACTVTY.length;j++)
						{
						var CurntACTVTY=ACTVTY[j];
						WFACTVINIT = WFActvInit(res.split("~")[2],res.split("~")[1]+"|"+CurntACTVTY+"|Vendor||ADMIN","LSW_SWFACTVINITCALL");
						var ChkFlowInit = UI_getdata(res.split("~")[1],CurntACTVTY,"","","","LSW_SCHKACTVITYASSIGNPROP");
						...
						var getBranchOPID = UI_getdata(res.split("~")[2],res.split("~")[1],"","","","LSW_SGETBRANCHOPSID");
						$(location).attr('href',window.location.origin + "/TPLSW/"+Page+"?PrcsID="+res.split("~")[1]+"&ActvID="+$(getBranchOPID).find("ID").text()+"&PrMs9=FormPageTab1&PrMs10=FormMainTab1");         	
						}
	            	}
	            else
	            	{
...
today = dd+'/'+mm+'/'+yyyy;
	...
	FncallMyAppl(this,'Table3',{spname:'LSW_SMYAPPLICATIONDTLSQ',DBSrc:'currentProfile',TableHeader:'card-headerGridAsh',Mode:'',Param:$('#LogUsr').val(),brid:$('#DMY4').val().split("|")[0]+'|'+$('#FromDate').val()+'|'+$('#ToDate').val()+'|'+$('#ApplNo').val(),MnuId:$("#VERTICAL").val()},{0:$('#LOCC_BrID'),1:$('#LOCC_BrName')},'||3,8',$(evnt).attr("value"));
	...
	else if ($(evnt).attr("value") == "VENDOR")
	{
		var hdnval = "";
		var pattern = /RCU/;
		var exists = pattern.test($('#DMY1').val());
		if(exists) {
			hdnval = "3,8";		
		}
		FncallMyAppl(this,'Table3',{spname:'LSW_SMYAPPLICATIONDTLS2_VENDR',DBSrc:'currentProfile',TableHeader:'card-headerGridAsh',Mode:'',Param:$('#LogUsr').val(),brid:'Loan',MnuId:$('#DMY1').val()+'|'+$('#GridCategory').val()+'|'+$("#VERTICAL").val()},{0:$('#LOCC_BrID'),1:$('#LOCC_BrName')},'||'+hdnval,$(evnt).attr("value"));
	}
	else if ($(evnt).attr("value") == "MigratedData")
	{
		FncallMyAppl(this,'Table3',{spname:'LSW_SMYAPPLICATIONDTLS2_MIGR',DBSrc:'currentProfile',TableHeader:'card-headerGridAsh',Mode:'',Param:$('#LogUsr').val(),brid:'Loan',MnuId:$("#VERTICAL").val()},{0:$('#LOCC_BrID'),1:$('#LOCC_BrName')},'||',$(evnt).attr("value"));
	}
}
