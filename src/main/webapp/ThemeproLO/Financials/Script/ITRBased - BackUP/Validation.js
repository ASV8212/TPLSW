...
function CheckLoan(){
	var check=UI_getdata($("#PrcsID").val(),"","","","","LSW_SGETLOANDETAILS");
...
$(document).on("blur",".SUB",function(){
	var sub=$("#CRTR_EXISTINGLNAMT").val();
	var sub2=$("#CRTR_CURRENTOUTST").val();
	if(sub=='')
	{
		sub=0;
...
$(document).on("blur",".INMUL",function(){
 var mul=$("#CRTR_COMPLETELNAMT").val();
 var mul1=$("#CRTR_MULTIPLIERLNAMT").val();
