  ...
  if(Activityname=='SendToCredit')
	...
	if(($("#DMY5").val().split("|")[1]=="Yes")||$("#PrMs1").val()=="View")
	 ...
	 $('.APIBUT').hide()	
	  if(Activityname=='SendToCredit')
	  {	
	var APIDETAILS=UI_getdata($("#PrcsID").val(),"","","","","LSW_SCHKUCVAPIDETAILS")
    var APIFLAG=$(APIDETAILS).find('UCVFLAG').text()
    if(APIFLAG=="YES")
	{
	 //DSVLBLALL('')	
	 $("#APIBUTTON").show()
...
$(document).on("click", ".DELETEFIRMDETAILS" , function() {
 ...
 $(document).on("click", ".DeleteSanct" , function() {
    ...
    $(document).on("click", "#Reject", function() {
		  ...
		  $(document).on("click", "#APIREJECT", function() {
			$("#REJECTREMARKSPopup").click();
       ...
       if($(this).text() == "Send for Approval")
	    ...
	    var BALANCEDATA=["ApproveNote2|"];
		 for (j=0;j<BALANCEDATA.length;j++)
			...
			else if($(this).attr('data-form')=="CIBILJust")
			{
			 $("#cke_AAPR_CIBILJUSTI").find(".cke_toolbar").find(".cke_button__save").click()
			}
   ...
   // });
    ...
    $("#BTNLOADBNKMULTIDATA").on('click', function() {
		...
		if($("#VERTICAL").val() == "UCV Eco" && $("#PrMs9").val() == "FormPageTab4" && $("#PrMs10").val() == "FormMainTab7" && $("#DMY7").val().split("|")[19] == "Y")
		 {
			 $("#APBD_ACCNTNO").material_select("destroy");
			 $("#APBD_ACCNTNO").removeAttr("disabled");
			 $("#APBD_ACCNTNO").material_select();
			 $("#SaveBNK").show();
		 }
    	if(CHKresult == "Fail")
		{
