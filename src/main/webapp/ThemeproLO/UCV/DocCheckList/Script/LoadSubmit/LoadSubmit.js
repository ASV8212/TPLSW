$(document).ready(function () {
	...
	//  {
	 ...
	 var op = UI_getdata($("#LogUsr").val(),$("#PrcsID").val(),$(".FormPageMultiTab li.active").attr("id"),"","","LSW_SGETUSRGRP1")
...
/* var Resi=$($('.BankDetail1').find(event.target).closest('.DYNROW')[0]).find("input[name='DOCP_PROPERTYDOC']:checked").val();

if(Resi=="")
 { *//* 
	 if($('#DOCP_PROPERTYDOC'+row).val() == "")
	 {
			//$(HTML).find(".UPLD").hide(); 
...
if($("#DOCP_PROPERTYDOC").val()=='')
	...
	{
		if($($('.BankDetail2').find('.DYNROW')[i]).find('[name=DOCP_PROPERTYDOC]').val() == '')
		 {
		   //$(".NBNKD").hide();
	...
	if($("#VERTICAL").val()=="UCV Eco")
	{
	 ...
	 if(($(op).find("groupId").text()=="BSM")&&(activityname1=="PreLogin"))
		 {
		 $("#HIDDENCOL").val('9,10,11,12,14,15')
		 FncallDocChkLst(this,'Table2',{spname:'LSW_SUCVKYCDOCCHKLST1',DBSrc:'currentProfile',TableHeader:'card-headerGridAsh',Mode:'',Param:'KYCDOC',brid:$('#PrcsID').val(),MnuId:$('#UPDC_CUSID').val()},{0:$('#LOCC_BrID'),1:$('#LOCC_BrName')},'||'+$("#HIDDENCOL").val(),'DocumentCheckList');
		 FncallDocChkLst(this,'Table3',{spname:'LSW_SUCVKYCDOCCHKLST1',DBSrc:'currentProfile',TableHeader:'card-headerGridAsh',Mode:'',Param:'Others',brid:$('#PrcsID').val(),MnuId:$('#UPDC_CUSID').val()},{0:$('#LOCC_BrID'),1:$('#LOCC_BrName')},'||'+$("#HIDDENCOL").val(),'DocumentCheckListOthers');
	...
	if(chk=='Y')	
	{
      ...
      else
      {
	    ...
	    $('.BankDetail2').find('.DOCPMndtry').addClass('DOCPMndtry');
      }
	}
	else
	{
	...
	$('.FormSave').on('click', function() {
		...
		if(MndtryChk == "Mandatory")
		 ...
		 // NXTTAB(this);
		 if($("#DMY7").val().split('|')[10] != "HEGECL" && $("#DMY5").val().split('|')[2] == "PreLogin"){
	...
	/*	if($(this).text() == "Send for Approval")
		...
		*/
		if($(this).text() == "Send to Credit" || $(this).text() == "Send for Approval" || $(this).text() == "Save & Submit")
			...
			if(MndtryChk != "")
				...
				var chkqry3=UI_getdata($("#PrcsID").val(),"","","","","LSW_SCHKQUERYSTATUS")
					chkqry3=$(chkqry3).find('RESULT').text()
				if(chkqry3 != 'SUCCESS')
						{
						  
						  var Alert2=''
								var nameArr = chkqry3.split(',')
						         k=nameArr.length
								 for(i=0;i<k;i++)
								 {
									var Alert1= nameArr[i]
		...
		// Mandatory Check End
			...
			{
				var chkqry2=UI_getdata($("#PrcsID").val(),"","","","","LSW_SCHKQUERYSTATUS")
					chkqry2=$(chkqry2).find('RESULT').text()
				if(chkqry2 != 'SUCCESS')
						{
						  
						  var Alert2=''
								var nameArr = chkqry2.split(',')
						         k=nameArr.length
								 for(i=0;i<k;i++)
								 {
									var Alert1= nameArr[i]
	   ...
	   WFComplete ($("#ActvID").val(),"var_status="+Button+"&var_RcmdUsr=CM","");
				...
				{ 
				   var xml=UI_getdata($("#PrcsID").val(),"","","","","LSW_SCHECKPFCOLLECTION")
					...
					if(Collection != 'SUCCESS')
						...
						var chkqry1=UI_getdata($("#PrcsID").val(),"","","","","LSW_SCHKQUERYSTATUS")
					chkqry1=$(chkqry1).find('RESULT').text()
				if(chkqry1 != 'SUCCESS')
						{
						  
						  var Alert2=''
								var nameArr = chkqry1.split(',')
						         k=nameArr.length
								 for(i=0;i<k;i++)
								 {
									var Alert1= nameArr[i]
		   ...
		   // var obj = JSON.parse(JSON.stringify(stm))
			 ...
			 if(($(this).text() == "Send Back") && (activityname == "DEFAppr" || activityname == "PLVer"))
				 {
					 if(activityname == "DEFAppr")
					 {
				        UI_getdata($("#PrcsID").val(),activityname,"","","","LSW_SUPDTPFSTST");
			 ...
			 if(($(this).text() == "Save & Submit") && (activityname == "PLVer"))
			 {
				 ...
				 var chkqry4=UI_getdata($("#PrcsID").val(),"","","","","LSW_SCHKQUERYSTATUS")
					chkqry4=$(chkqry4).find('RESULT').text()
				if(chkqry4 != 'SUCCESS')
						{
						  
						  var Alert2=''
								var nameArr = chkqry4.split(',')
						         k=nameArr.length
								 for(i=0;i<k;i++)
								 {
									var Alert1= nameArr[i]
	...
	$(document).on("change", ".VRFY", function() {
		var secid = $(this).attr("data-quey-sec");
			...
			var op=""
				...
				if(secid=="S01")
				{
					op=UI_getdata("Document Checklist",$("#PrcsID").val(),$($(this).closest('.tbodytr').find('.tbodytrtd')[9]).text()+'-'+$($(this).closest('.tbodytr').find('.tbodytrtd')[1]).text()+'-'+$($(this).closest('.tbodytr').find('.tbodytrtd')[2]).find('[name=KYCD_DOCNAME]').val(),"","","LSW_SCHKQURYRAISED");
				}
				if(secid=="S02"){
		...
		else
{
    	...
    	if(Val == "Deferred")
    		...
    		$($(this).closest('.tbodytr').find('input[type=checkbox]')[0]).removeClass("KYCDGRDMNDTRY");
				...
				$($(this).closest('.tbodytr').find('input[type=checkbox]')[0]).removeClass("OKYDGRDMNDTRY");
			}
    		}
    	else {
    		...
    		$($(this).closest('.tbodytr').find('input[type=checkbox]')[0]).addClass("KYCDGRDMNDTRY");
				...
				$($(this).closest('.tbodytr').find('input[type=checkbox]')[0]).addClass("OKYDGRDMNDTRY");
			}
    	}
}
else{
	...
	/*var xml=UI_getdata($("#PrcsID").val(),$('#LogUsr').val(),"","","","LSW_SGETQUERYTO");*/
		...
		{
			$("#MNGQ_QURYNAME").val($($(this).closest('.tbodytr').find('.tbodytrtd')[1]).text());
			$("#MNGQ_UNIQID").val($($(this).closest('.tbodytr').find('.tbodytrtd')[9]).text()+'-'+$($(this).closest('.tbodytr').find('.tbodytrtd')[1]).text()+'-'+$($(this).closest('.tbodytr').find('.tbodytrtd')[2]).find('[name=KYCD_DOCNAME]').val());
		}
		if(secid=="S02"){
