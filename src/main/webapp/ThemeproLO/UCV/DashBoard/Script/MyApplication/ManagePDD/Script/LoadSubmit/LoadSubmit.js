$(document).ready(function () {
	...
	$(document).on("click", ".FormSave", function() {
		...
		var x = document.getElementsByClassName("RESTDBfields");
    ...
    for (i = 0; i < x.length; i++) {
		...
		if($(document.getElementsByClassName("RESTDBfields").item(i)).is("select")==true || $(document.getElementsByClassName("RESTDBfields").item(i)).is("input")==true || $(document.getElementsByClassName("RESTDBfields").item(i)).is("textarea")==true){
		
        name = document.getElementsByClassName("RESTDBfields").item(i).name;
        fieldid = document.getElementsByClassName("RESTDBfields").item(i).id;
	  
	    value = document.getElementsByClassName("RESTDBfields").item(i).value 
	 ...
	 formxml = formxml + "</Details></Form>";
	   ...
	   var  LEADID = UI_getdata("LeadCr","s","","","","LSW_Stestleads3");
		var REST_LEADID=$(LEADID).find("REST_LEADID").text();
		var LoanType = UI_getdata(REST_LEADID,formxml,"","","","LSW_Stestleads10");
		...
		else if($(LoanType).find("NAMEEMAILEXISTS").text()=='NAME & PAN ALREADY EXISTS')
		{
			alert("NAME & PAN ALREADY EXISTS");
			return;
		...
		else if($(LoanType).find("PANEXISTS").text()=='PAN ALREADY EXISTS')
		{
			alert( "");
			return;
