...
//$(".InitWF1").click(function () {
			...
			$(location).attr('href',window.location.origin + "/TPLSW/Applcnt?PrcsID="+$($(this).parent().parent().find('td')[10]).text()+"&ActvID="+$($(this).parent().parent().find('td')[11]).text()+"&PrMs9=FormPageTab1&PrMs10=FormMainTab1");
			}
			else{
		...
		$(document).on("click",".InitWF2",function() {
			...
			$(location).attr('href',window.location.origin + "/TPLSW/Disbrsmnt?PrcsID="+$($(this).parent().parent().find('td')[7]).text()+"&ActvID="+ActvIDQD+"&PrMs9=FormPageTab1&PrMs10=FormMainTab8");
...
today = dd+'/'+mm+'/'+yyyy;
	...
	{	
	 FncallMyAppl(this,'Table3',{spname:'LSW_SMYAPPLICATIONDTLS2',DBSrc:'currentProfile',TableHeader:'card-headerGridAsh',Mode:'',Param:$('#LogUsr').val(),brid:$('#DMY4').val().split("|")[0],MnuId:$('#DMY1').val()+'|'+$('#GridCategory').val()},{0:$('#LOCC_BrID'),1:$('#LOCC_BrName')},'||3',$(evnt).attr("value"));
	}
	if ($(evnt).attr("value") == "GENERALQUEUE")
	{
	
	FncallMyAppl(this,'Table3',{spname:'LSW_SMYAPPLICATIONDTLSQ',DBSrc:'currentProfile',TableHeader:'card-headerGridAsh',Mode:'',Param:$('#LogUsr').val(),brid:$('#DMY4').val().split("|")[0]+'|'+$('#FromDate').val()+'|'+$('#ToDate').val()+'|'+$('#ApplNo').val(),MnuId:''},{0:$('#LOCC_BrID'),1:$('#LOCC_BrName')},'||3',$(evnt).attr("value"));
