$(document).ready(function () {
	...
	$(".btnDashsmGicn").click(function () {
		
		var indx=$(this).closest('td').index();
		
		var headval=$($(this).closest('td').closest('table').find('thead th')[indx]).text();
		var val=$($(this).closest('tr').find('th')[0]).text();
		if(val=='')
		{
			val=$($(this).closest('table').closest('div').parent('div').parent('div').prev().find('div')[0]).text().trim()

		}
		else{
val=val.replace(val. match(/(\d+)/g)[0], '').trim()
		}
	$("#ACTNNAME").val(val+' : '+headval)
	
		$("#ACTNBRPopup").click();
		FncallMyApplDshBrd(this,'Table11',{spname:"LSW_SACTNABLEDASHGRIDPOUP",DBSrc:"currentProfile",TableHeader:"card-headerGridAsh",Mode:"",Param:$("#LogUsr").val(),brid:headval,MnuId:val},{0:"",1:""},"||","ACTNBLEDSHBRDAPPLNO");


	
	    });
	
	$(".btnDashsmYicn").click(function () {
			var indx=$(this).closest('td').index();
