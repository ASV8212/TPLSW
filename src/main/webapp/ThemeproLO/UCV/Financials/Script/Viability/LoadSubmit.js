...
$(document).ready(function () {
	...
	$("#VIAA_PRCSID").val("#prcsid");
	$("#VIAA_ACTIVITYID").val("#ActvID");
    FormDataFromDB("LSW_TVIABDETL","VIAA_", "VIAADBfields","");
	 
	
	
	//$("#BTNVIABILITYGRD").click();
	$("#BTNVIABINCOMEGRD").click();
	$("#BTNVIABEXPENSEGRD").click();
	Chkemi();
	
	//$("#BTNVARIASALGRD").click();
   // $("#BTNDEDUCTIONGRD").click();
	//$("#BTNOTHERINCMGRD").click();
	...
	//$('.FormSave').on('click', function() {
		...
		/*var MndtryChk = ChkMandatoryFlds_V1("VIINMNDTRY",html);
		...
		if(MndtryChk == "Mandatory")
			...
			/*MndtryChk = CheckDocMndtry("Table4","VIINMNDTRY","Others");
		if(MndtryChk != "")
			{
			alert(MndtryChk);
			return;
        ...
        //}
			...
			var VIAA_INCOME = TxtGridsubmitdata_V1("Table4","VIIN_","VIAA_");
			AssignGridXmltoField("VIAA_INCOME", VIAA_INCOME)
			
			var VIAA_EXPENSE = TxtGridsubmitdata_V1("Table5","VIEX_","VIAA_");
			AssignGridXmltoField("VIAA_EXPENSE", VIAA_EXPENSE)
	...
	UI_getdata($("#PrcsID").val(),$(".FormPageMultiTab li.active").text(),$(".FormPageMultiTab li.active").attr("id"),"","","LSW_SSTRCAMMAINTABLE")
        ...
        if ($(this).text() == "Save & Next") {
			if($(".CHKVAL").val()<=0)
			{
				alert('Negative value or zero value generated. Kindly enter the correct value');
				$(".CHKVAL").val('0')
				return false;
			}
            NXTTAB(this);
