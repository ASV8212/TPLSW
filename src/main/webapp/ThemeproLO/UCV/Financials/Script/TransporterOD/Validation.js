...
$(document).on("blur",".OVEREXT",function(){
	var NoOfVehicle=$("#CTOD_NOOFVEHICLE").val(); 
	var EstIncm=$("#CTOD_ESTIINCM").val().replace(/,/g,'');  
	if(NoOfVehicle=='')
	{
		NoOfVehicle=0;
	}
	if(isNaN(NoOfVehicle))
	{
		NoOfVehicle=0;
	}
	if(EstIncm=='')
	...
	var OverAll=parseFloat(NoOfVehicle)*parseFloat(EstIncm)*12;
	OverAll=CURINRCommaSep(parseFloat(OverAll).toFixed(0));
	$("#CTOD_OVERALLEST").val(OverAll);
	$("#CTOD_OVERALLEST").next().addClass('active');
	
	    OverAll=OverAll.replace(/,/g,'')
	
	var TotElig= parseFloat(OverAll)*20/100
	TotElig=CURINRCommaSep(parseFloat(TotElig).toFixed(0));
	$("#CTOD_OVERDRFTLN").val(TotElig);
	$("#CTOD_OVERDRFTLN").next().addClass('active');
	
	TotElig=TotElig.replace(/,/g,'')
	
	var ExODLon=$("#CTOD_EXISTOVRDFT").val().replace(/,/g,'');
	...
	var NetElig = parseFloat(TotElig)-parseFloat(ExODLon);
	
	NetElig=CURINRCommaSep(parseFloat(NetElig).toFixed(0));
	$("#CTOD_NETELIOVRDFT").val(NetElig);
	$("#CTOD_NETELIOVRDFT").next().addClass('active');
	
	var NetEligLon=$("#CTOD_NETELINLAMT").val().replace(/,/g,'');
	
	var NetEligODLon=$("#CTOD_NETELIOVRDFT").val().replace(/,/g,'');
	...
	if(isNaN(NetEligODLon))
	{
		NetEligODLon=0;
	}
	
	var FinalLonElig=Math.min(NetEligLon, NetEligODLon)
	
	FinalLonElig=CURINRCommaSep(parseFloat(FinalLonElig).toFixed(0));
	$("#CTOD_FNLLNELI").val(FinalLonElig);
	$("#CTOD_FNLLNELI").next().addClass('active');
})