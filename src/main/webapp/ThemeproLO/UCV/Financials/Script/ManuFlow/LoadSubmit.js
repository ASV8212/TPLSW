$(document).ready(function() {
    ...
    $("#MANU_PRCSID").attr("value", $("#PrcsID").val());
	  GetBorowClass();
      GetSector();
    FormDataFromDB("LSW_TMANUALFLOW", "MANU_", "MANUDBfields", "");
...
$("#MANU_UNIQID").attr("value",$(".FormPageMultiTab li.active").attr("id"));
	...
	var OP=UI_getdata($("#PrcsID").val(),$(".FormPageMultiTab li.active").text(),$(".FormPageMultiTab li.active").attr("id"),"","","LSW_SSTRCAMMAINTABLE")
      if($(OP).find("LOANID").text()!="")
		  {
		  $("#MANU_LOANID").val($(OP).find("LOANID").text());
		  $("#RCCM_LOANUNIQID").val($(OP).find("LOANID").text());
		  }
		  
		  
		   var CHKresult=FormDataToDB(tbl,prfx,$("#MANU_PRCSID").val()+"|"+$("#"+DATA).val()+"|" + DATA);
			if(CHKresult == "Fail")
	   		{
