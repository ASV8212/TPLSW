        ...
        $(table.body()).on('click', 'td', function () {
            ...
            if ((settings.columns && settings.columns.indexOf(currentColumnIndex) > -1) || (!settings.columns)) {
                ...
                if (!$(cell).find('input').length && !$(cell).find('select').length && !$(cell).find('textarea').length) {
                    ...
                    $(document).find(".GridSelect").material_select('destroy');
                    $(document).find('.GridSelect').material_select();
                    if (input.focus) {
...
function getInputHtml(currentColumnIndex, settings, oldValue) {
    ...
    switch (inputType) {
        case "list":
            ...
            input.html = startWrapperHtml + "<select class='" + inputCss + " GridSelect' onchange='$(this).updateEditableCell(this);'>";
        ...
        case "list-confirm": // List w/ confirm
           ...
           // input.html = startWrapperHtml + "<select class='" + inputCss + " GridSelect'>";
        	 input.html = startWrapperHtml + "<select class='" + inputCss + " GridSelect' onchange='$(this).updateEditableCell(this);'>";
        	$.each(inputSetting.options, function (index, option) {
                if (oldValue == option.value) {
                   input.html = input.html + "<option value='" + option.value + "' selected>" + option.display + "</option>"
        ...
        case "radio":
            ...
            input.html = startWrapperHtml + "<div class='" + inputCss + " select-radio' onchange='$(this).updateEditableCell(this);'>";
            $.each(inputSetting.options, function (index, option) {
                if (oldValue == option.value) {
                	var html = "<div class='custom-control custom-radio custom-control-inline'><input type='radio' onclick='$(this).updateEditableCell(this);' checked='checked' class='custom-control-input' value='"+option.value+"' id='"+option.display+"' name='"+option.name+"'><label class='custom-control-label' for='"+option.display+"'>"+option.value+"</label></div>";
                	input.html = input.html + html;
                } else {
            	var html = "<div class='custom-control custom-radio custom-control-inline'><input type='radio' onclick='$(this).updateEditableCell(this);' class='custom-control-input' value='"+option.value+"' id='"+option.display+"' name='"+option.name+"'><label class='custom-control-label' for='"+option.display+"'>"+option.value+"</label></div>";
                   input.html = input.html + html;
                }
            });
            input.html = input.html + "</div>" + endWrapperHtml;
            input.focus = false;
            ...
            break;
        case "radio-confirm": // List w/ confirm
        
        	  input.html = startWrapperHtml + "<div class='" + inputCss + " select-radio' >";
              $.each(inputSetting.options, function (index, option) {
            	  if (oldValue == option.value) {
                  	var html = "<div class='custom-control custom-radio custom-control-inline'><input type='radio' onclick='$(this).updateEditableCell(this);' checked='checked' class='custom-control-input' value='"+option.value+"' id='"+option.display+"' name='"+option.name+"'><label class='custom-control-label' for='"+option.display+"'>"+option.value+"</label></div>";
                  	input.html = input.html + html;
...
function getInputField(callingElement) {
    ...
    switch ($(callingElement).prop('nodeName').toLowerCase()) {
        case 'a': // This means they're using confirmation buttons
            ...
            if ($(callingElement).siblings('.GridSelect').length > 0)
            	{
            	inputField = $(callingElement).siblings('.GridSelect').find('select');
            	}
