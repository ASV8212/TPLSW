$(document).ready(function () {
 ...
 var CATAGORY=$("#URCR_BRMAPPINGCA").val()
	...
	$(document).on("click", "#RptUsrModalSmt" , function() {
		...
		$(document).on("click", "#UserMODULEModalSmt" , function() {
		
		var data1=popTableModPageGrid1.$('input:checkbox:checked').map( function () {
		return $(this).attr('title');
			}).get().join();
			
		var op = UI_getdata(data1,"MODULE",'<Data><row><PRCSID>'+$("#PrcsID").val()+'</PRCSID><ACTVTYID>'+$("#ActvID").val()+'</ACTVTYID><UNIQID>'+$("#MODLU_UNIQID").val()+'</UNIQID></row></Data>',"","","LSW_SPUSHDATATOTEMPGTBL")
		if($(op).find("RESULT").text()=="SUCCESS"){	
	    $("#BTNMODULEGRD").click();
			$("#UserMODULEModalClose").click();
