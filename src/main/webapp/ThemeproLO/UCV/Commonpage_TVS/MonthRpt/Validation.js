...
alert('Invalid File Format');
 ...
 return;	
 	...
 	var   param3="LSW_TDASHBOARDUPLOAD";
 		...
 		$.ajax({
 	        ...
 	        success: function (xml1) {
 				...
 				if(xml1=='Success')
 				{
					...
					var VALIDATIONCHK=UI_getdata($(op).find("VR").text(),"","","","","LSW_SDASHUPLOADVALIDATION")
					
					if($(VALIDATIONCHK).find('RESULT').text()!="SUCCESS")
					{
				        var VALIDATIONRST=$(VALIDATIONCHK).find('RESULT').text()
				                var Alert2=''
								var nameArr = VALIDATIONRST.split('.')
						         k=nameArr.length
								 for(i=0;i<k;i++)
								 {
					...
					var TargetIns=UI_getdata("","","","","","LSW_sDASHBRDTRGTINS");
			...
			//	FncallDocChkLst(this,'Table2',{spname:'LSW_SMonthlyRPT',DBSrc:'currentProfile',TableHeader:'card-headerGridAsh',Mode:'',Param:$(op).find("VR").text(),brid:'',MnuId:''},{0:$('#LOCC_BrID'),1:$('#LOCC_BrName')},'||','MonthRpt')
				...
				$("#MORP_RPTTYPE").val('SALES')
				$('#MORP_RPTTYPE').material_select();	
                $("#MORP_ROLE").empty();
                $("#MORP_ROLE").append('<option value="" selected>--Select--</option><option value="BSM">BSM</option><option value="ASM">ASM</option><option value="RSM">RSM</option><option value="ZSM">ZSM</option><option value="NSM">NSM</option>') 
				$('#MORP_ROLE').material_select("destroy");	
				$('#MORP_ROLE').material_select();	
				$('#MORP_ROLE').val('BSM');	
				$('#MORP_ROLE').material_select();	
...
function GridControlMonthREPORT (popTableModPageGrid1,TableID,dtData,dtcolumn,hideClm)
{
...
function ROLEDROPDOWN()
{
	...
	if($("#EXPT_RPTTYPE").val()=="CREDIT")
	{		
	$("#EXPT_ROLE").empty();
    $("#EXPT_ROLE").append('<option value="" selected>--Select--</option><option value="BCM">BCM</option><option value="ACM">ACM</option><option value="RCM">RCM</option><option value="ZCM">ZCM</option><option value="NCM">NCM</option>') 
    $('#EXPT_ROLE').material_select("destroy");	
    $('#EXPT_ROLE').material_select();
	}
	else if($("#EXPT_RPTTYPE").val()=="SALES")
	{
	$("#EXPT_ROLE").empty();
    $("#EXPT_ROLE").append('<option value="" selected>--Select--</option><option value="BSM">BSM</option><option value="ASM">ASM</option><option value="RSM">RSM</option><option value="ZSM">ZSM</option><option value="NSM">NSM</option>') 
    $('#EXPT_ROLE').material_select("destroy");	
    $('#EXPT_ROLE').material_select();	
...
function MONTHLYROLEDROPDOWN()
{
	
if($("#MORP_RPTTYPE").val()=="CREDIT")
	{		
	$("#MORP_ROLE").empty();
    $("#MORP_ROLE").append('<option value="" selected>--Select--</option><option value="BCM">BCM</option><option value="ACM">ACM</option><option value="RCM">RCM</option><option value="ZCM">ZCM</option><option value="NCM">NCM</option>') 
    $('#MORP_ROLE').material_select("destroy");	
    $('#MORP_ROLE').material_select();
	}
	else if($("#MORP_RPTTYPE").val()=="SALES")
	{
	$("#MORP_ROLE").empty();
    $("#MORP_ROLE").append('<option value="" selected>--Select--</option><option value="BSM">BSM</option><option value="ASM">ASM</option><option value="RSM">RSM</option><option value="ZSM">ZSM</option><option value="NSM">NSM</option>') 
    $('#MORP_ROLE').material_select("destroy");	
