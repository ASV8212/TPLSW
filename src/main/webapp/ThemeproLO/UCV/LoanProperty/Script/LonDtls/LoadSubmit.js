...
$(document).ready(function () {
	...
	var LonPurpose = UI_getdata("PurposeLoan","",$("#VERTICAL").val(),"","","LSW_SGETPRODVAL");
	$("#LODE_LONPURP").html("")
	$("#LODE_LONPURP").append($(LonPurpose).find("RESULT").html());
	$("#LODE_LONPURP").material_select();
	
	var SubLon = UI_getdata("LoanPurpose","",$("#VERTICAL").val(),"","","LSW_SGETPRODVAL");
	$("#LODE_SUBLONPURP").html("")
	$("#LODE_SUBLONPURP").append($(SubLon).find("RESULT").html());
...
//	GetNaturProduct();
	...
	if($("#VERTICAL").val()=="UCV Eco"||$("#VERTICAL").val()=="UCV")
	{
	  $(".FormPageMultiTabAdd").hide();
	}
	...
	if($("#VERTICAL").val()=="UCV Eco")
	{
	  $(".FormPageMultiTabAdd").hide();
	  $(".UCVECOHIDE").hide();
	  $('input:radio[name=LODE_SECURED][value=Unsecured]').prop("checked", true);
	}
	if($("#VERTICAL").val()=="UCV Eco")
	{
		$("#LODE_CRDTSHELD").removeClass("LODEMndtry");
		$("#LODE_CRDTSHELD").closest("div").next().find(".MndtryAstr").html("")
   ...
   $.ajax({
			...
			*/
			...
			if($(this).text() == "Save & Next")
			{
				if($($(".FormPageTab li")[1]).is(":visible") == true)
				{
					$($(".FormPageTab li")[1]).click();
				}
				else{
					if($($(".FormPageTab li")[2]).is(":visible") == true)
					{
						$($(".FormPageTab li")[2]).click();
					}
					else{
