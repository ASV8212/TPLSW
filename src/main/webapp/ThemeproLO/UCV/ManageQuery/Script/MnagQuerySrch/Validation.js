...
"aoColumnDefs": [ 
 ...
 function DropDownChange(Reg,Area,branch)
 {
  ...
  function ReTgrGrd(){
	  var val=""
	  for(var i=0;i<$('.CHKFLD').not('div').length;i++){
		  if(val ==""){
			  val =$($('.CHKFLD').not('div')[i]).val()
		  }
	  }
	  FncallMyAppl(this,'Table3',{spname:'LSW_SMANGQRY',DBSrc:'currentProfile',TableHeader:'card-headerGridAsh',Mode:'',Param:$('#LogUsr').val(),brid:$('#DMY4').val().split("|")[0],MnuId:val},{0:$('#LOCC_BrID'),1:$('#LOCC_BrName')},'||','MANAGEDEFR');
	  //FncallFEPDashLrgDataWP(this,'Table3',{spname:'LSW_SMANGQRY',DBSrc:'currentProfile',TableHeader:'card-headerGridAsh',Mode:'',Param:$('#LogUsr').val(),brid:'',MnuId:val},{0:$('#LOCC_BrID'),1:$('#LOCC_BrName')},'||');
  }
