...
function CheckPan()
{
		...
		else
	{
		/* if($("#CBSI_PAN").val()=="")
		{
			$("#CEMI_INCCONSID").attr('disabled',true);
			$(html).find("#CEMI_INCCONSID1").removeClass("CEMIMndtry");
			//$("#CEMI_INCCONSID1").attr('disabled',true);
			//$("#CEMI_INCCONSID1").removeClass("CEMIMndtry");
		}
		else
		{
			$("#CEMI_INCCONSID").attr('disabled',false);
			$(html).find("#CEMI_INCCONSID1").addClass("CEMIMndtry");		
			//$("#CEMI_INCCONSID1").attr('disabled',false);
			//$("#CEMI_INCCONSID1").addClass("CEMIMndtry");
		} */
	}
...
/* function InactivePanCheck()
{
	if($("#KARZASTATUS").val()=='Inactive')
	{
		if($("#CBSI_PAN").val()=="")
		{
		 ...
		 //$("#CEMI_INCCONSID").attr('disabled',true);
		 //$("#CEMI_INCCONSID").removeClass("CEMIMndtry");
		 $("#CEMI_INCCONSID").attr('disabled',true);
		 $(html).find("#CEMI_INCCONSID1").removeClass("CEMIMndtry");	
		}
		else
		{
		 ...
		 $("#CEMI_INCCONSID").attr('disabled',false);
		// $("#CEMI_INCCONSID").addClass("CEMIMndtry");
		$(html).find("#CEMI_INCCONSID1").addClass("CEMIMndtry");	
		}
	}
} */
...
//var Resi=$($('.BankDetail1').find(event.target).closest('.DYNROW')[0]).find("input[name='CEMI_ECUAMNO']:checked").val();
    ...
    if(borrower=="Yes")
	{
	if($("#CEMI_ECUAMNO"+row).prop('checked') == true){
	//	if($($('.BankDetail1').find(event.target).closest('.DYNROW')[0]).find("input[name='CEMI_ECUAMNO']:checked") == true){
		$("#CEMI_EUAMNO"+row).val('');
		$("#CEMI_EUAMNO"+row).removeClass("CEMIMndtry");
		$("#CEMI_EUAMNO"+row).next().find(".MndtryAstr").html("");
	}
	else {

		$("#CEMI_EUAMNO"+row).addClass("CEMIMndtry");
		$("#CEMI_EUAMNO"+row).next().find(".MndtryAstr").html("*");
	}
	}
	else
	{
		...
		$("#CEMI_EUAMNO"+row).removeClass("CEMIMndtry");
		$("#CEMI_EUAMNO"+row).next().find(".MndtryAstr").html("");
		$("#CEMI_EUAMNO"+row).attr("disabled",true);
		$(".veri").hide();	
	}
...
/* function CheckIncomeInd(Evnt)
{
  var html=$('.BankDetail1').find(".DYNROW")
	if($('#'+$(Evnt).attr('id')).prop('checked') == true)
	{
		$(html).find(".SEGM").show();
		//$(html).find("[name='CEMI_PROFILE']").material_select(); 
		$(html).find(".DAT").addClass("CEMIMndtry");	
		$(html).find(".DAT").next().find(".MndtryAstr").html("*");
	}
	else
	{
		$(html).find(".SEGM").hide();
		$(html).find(".DAT").val('');
		$('input:radio[name=CEMI_CAPTIVEUSE]')[0].checked = false;
		$('input:radio[name=CEMI_CAPTIVEUSE]')[1].checked = false;
		$('input:radio[name=CEMI_SEGMENT]')[0].checked = false;
		$('input:radio[name=CEMI_SEGMENT]')[1].checked = false;
		//$(html).find("[name='CEMI_PROFILE']").html("")
		$(html).find("[name='CEMI_PROFILE']").material_select(); 	
		$(html).find(".DAT").next().removeClass('active');
		$(html).find(".DAT").removeClass("CEMIMndtry");	
		$(html).find(".DAT").next().find(".MndtryAstr").html("*");	
	}
}	
 */
/* function CheckIncomeLoad(html)
{

	if($(html).find("input[name=CEMI_INCCONSID]").prop('checked') == true)
	{
		$(html).find(".SEGM").show();
		$(html).find(".DAT").addClass("CEMIMndtry");	
		$(html).find(".DAT").next().find(".MndtryAstr").html("*");
...
function GetProfile()
{
	...
	$("#CEMI_PROFILE").html("")
	$("#CEMI_PROFILE").append($(Pro).find("RESULT").html());	
}

function GetProfileNonInd()
{
	var Pro = UI_getdata("PROFILE","","","","","LSW_SGETPRODVAL");
	
	$("#CSAP_PROFILE").html("")
	$("#CSAP_PROFILE").append($(Pro).find("RESULT").html());	
}

/* function CheckIncoNonIn()
{
	if($('#CDOG_INCCONSID').is(':checked')==true)
	{
	
	    
		$(".NONIND").show();
		$(".DOC").addClass("CDOGMndtry");	
		$(".DOC").next().find(".MndtryAstr").html("*");
	}
	else
	{
		$(".DOC").val('');
		$('input:radio[name=CDOG_SEGMENT]')[0].checked = false;
		$('input:radio[name=CDOG_SEGMENT]')[1].checked = false;
		$('input:radio[name=CDOG_CAPTIVEUSE]')[0].checked = false;
		$('input:radio[name=CDOG_CAPTIVEUSE]')[1].checked = false;
		$("#CDOG_PROFILE").material_select(); 
		$(".DOC").removeClass("CDOGMndtry");
		$(".DOC").next().find(".MndtryAstr").html("*");
		$(".NONIND").hide();
