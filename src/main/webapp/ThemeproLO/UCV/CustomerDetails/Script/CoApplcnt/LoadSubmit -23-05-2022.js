    	    ...
    	    if($(this).text()=="Confirm")
    	    	{
					...
					var personalinfocusid = $($("input[name='ACTION']:checked").closest(".tbodytr").find(".tbodytrtd")[14]).text();
					//var addrinfocusid = $($("input[name='ADDRACTION']:checked").closest(".tbodytr").find(".tbodytrtd")[12]).text();
					/*if(contactcusid == "NA" || personalinfocusid == "NA" || addrinfocusid == "NA")
					{
						alert("Invalid Customer ID Has Been Picked For Processing, Pick a Valid Customer ID to Process Further");
						return;
					}
					if(contactcusid == "" || personalinfocusid == "" || addrinfocusid == "")
					{
						alert("Invalid Customer ID Has Been Picked For Processing, Pick a Valid Customer ID to Process Further");
						return;
					}
   ...
   var SatetVal=$($("input[name='ACTION']:checked").closest('tbody tr').find('td')[12]).text()
  	  ...
  	  if($("#"+MartStatus).val() == "Married")
		 ...
		 if($("#COBI_CONSTITUTION").val()=="Proprietorship" || $("#COBI_CONSTITUTION").val()=="Society" || $("#COBI_CONSTITUTION").val()=="Trustee")
		  ...
		  if(contactcusid != "" && personalinfocusid != "" && addrinfocusid != "")
			{
	         UI_getdata($("#PrcsID").val(),$("#COBI_CUSID").val(),contactcusid,"","","LSW_SPUSHPOSIDEXTOCUSID_1");
			 $("#COBI_CUSID").val(contactcusid);
			 $("#COAI_CUSID").val(contactcusid);
			 $("#Save1").click();
			 $("#Save2").click();
		...
		if($($("#DedupeBIFTable").find(".tbodytr")[0]).find(".tbodytrtd").text() != "No data available in table")
		{
			...
			{
				if($($($("#DedupeBIFTable").find(".tbodytr")[i]).find(".tbodytrtd")[15]).text() == "Y")
				{
					chkyflg_1 = true;
					if($($($("#DedupeBIFTable").find(".tbodytr")[i]).find(".tbodytrtd")[8]).find("input[type=radio]").is(':checked') && chkyflg == "")
					{
						chkyflg = true;
					}
				}
