...
//    }
	...
	public String GSTServiceCall(String spname, String input1, String input2, String processid) throws Exception {
		...
		try {
			...
			if (colCount > 1) {
				...
				JSONData = JSONData.replace("encryptgstNumber", encryptgstNumber).replace("encryptusername",
						encryptusername);
	...
	public String gstUploadCall(String spname, String input1, String input2, String input3, String processid) throws Exception {
		String resp = "";
		Connection congst = null;
		...
		try {
			...
			if (colCount > 1) {
				...
				resp = InterfaceGSTAPI.InterfaceGSTUpload(URL, XMLData, contentType, vendorID);
				...
				psgst1 = congst1.prepareStatement("{ call " + spname + "_UPD" + "(?,?,?,?,?) }");
			...
			resp = "Exception~"+ex.getMessage();
			ex.printStackTrace();
		} finally {
			DBConnection.closeConnection(congst, psgst, rsgst);
			DBConnection.closeConnection(congst, psgst1, null);
		}
		return resp;
	}
	
	public String gstProcessCall(String spname, String input1, String input2, String processid) throws Exception {
		String resp = "";
		Connection congst = null;
		PreparedStatement psgst = null;
		ResultSet rsgst = null;
		...
		try
		{
			...
			while (rsgst.next()) {
				URL = rsgst.getString(1);
				Uniqref = rsgst.getString(2);
			}
			
			if (colCount > 1) {
				Security.addProvider(new org.bouncycastle.jce.provider.BouncyCastleProvider());
				
				resp = InterfaceGSTAPI.InterfaceGSTProcess(URL, contentType);
