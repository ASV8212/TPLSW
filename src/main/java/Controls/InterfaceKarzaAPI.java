...
public class InterfaceKarzaAPI
{
    public static String InterfaceKYCCall(final String APiURL, final String JsonData, final String contenttype, final String karzakey) {
        ...
        String Message = null;
        try {
            //final ConnectionSpec spec = new ConnectionSpec.Builder(ConnectionSpec.MODERN_TLS).tlsVersions(new TlsVersion[] { TlsVersion.TLS_1_2 }).cipherSuites(new CipherSuite[] { CipherSuite.TLS_DHE_RSA_WITH_AES_256_GCM_SHA384, CipherSuite.TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384, CipherSuite.TLS_DHE_RSA_WITH_AES_128_GCM_SHA256 }).build();
            final ConnectionSpec spec = new ConnectionSpec.Builder(ConnectionSpec.MODERN_TLS).tlsVersions(new TlsVersion[] { TlsVersion.TLS_1_2 }).cipherSuites(new CipherSuite[] { CipherSuite.TLS_DHE_RSA_WITH_AES_256_GCM_SHA384, CipherSuite.TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384}).build();
            final OkHttpClient client = new OkHttpClient();
            client.setConnectionSpecs((List)Collections.singletonList(spec));
            final MediaType mediaType = MediaType.parse(contenttype);
            final RequestBody body = RequestBody.create(mediaType, JsonData);
            final Request request = new Request.Builder().url(APiURL).post(body).addHeader("content-type", contenttype).addHeader("x-karza-key", karzakey).addHeader("cache-control", "no-cache").build();
            ...
            return String.valueOf(Status) + "!@#" + Message;
        }
        return String.valueOf(Status) + "!@#" + Message;
    }
    ...
    public static String InterfaceMultipartCall(final String APiURL, String Path, final String contenttype, final String karzakey, final String APIType, final String Param) throws FileNotFoundException, Exception {
        String Status = null;
        String Message = null;
        ...
        try {
        	...
        	final okhttp3.ConnectionSpec spc2 = new okhttp3.ConnectionSpec.Builder(okhttp3.ConnectionSpec.MODERN_TLS).tlsVersions(new okhttp3.TlsVersion[] { okhttp3.TlsVersion.TLS_1_2 }).cipherSuites(new okhttp3.CipherSuite[] { okhttp3.CipherSuite.TLS_DHE_RSA_WITH_AES_256_GCM_SHA384, okhttp3.CipherSuite.TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384 }).build();
            ...
            else if(APIType.equals("AADHAAR ZIP VERIFY"))
            {
            	...
            	String URL=InputPath[0];
            	String shareCode=InputPath[1];
            	String Mob=InputPath[2];
            	final byte[] bd = URLtoFile(URL);
            	final okhttp3.OkHttpClient client2 = new okhttp3.OkHttpClient.Builder().connectionSpecs((List)Arrays.asList(spc2)).build();
            	final okhttp3.MediaType mediaType2 = okhttp3.MediaType.parse(contenttype);
            	requestBody = (okhttp3.RequestBody)new MultipartBody.Builder().setType(MultipartBody.FORM).addFormDataPart("file", "image.jpeg/abc.pdf/ima.png/imag.jpg/test.xml/test.zip", okhttp3.RequestBody.create(mediaType2, bd)).addFormDataPart("shareCode",shareCode).addFormDataPart("mobile",Mob).addFormDataPart("consent", "y").build();
            	request = new okhttp3.Request.Builder().url(APiURL).post(requestBody).addHeader("x-karza-key", karzakey).build();
            	response = client2.newCall(request).execute();
            }
